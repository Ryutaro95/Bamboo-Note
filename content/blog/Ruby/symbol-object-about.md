...
僕が初めて学習したプログラミング言語がRubyでした。そのため「プログラミングにはシンボルというものがあって、文字列っぽいものなんだ」と曖昧な解釈をしていました。  

ただ、他の言語を学習してみるとRubyのシンボルのようなものは出てきませんでした。そこで改めてシンボルとは？についてまとめてみました。
...
まずはRubyの公式ドキュメントを確認したところ次のように説明されていました。  
...
個人的に公式ドキュメントを見ただけでは「分かりそうだけど分からん」という感じでした。  
まずは公式ドキュメントを整理してみたいと思います。  
...
シンボルは`Symbol`クラスに属していることが分かりました。  
...
シンボルは文字列と一対一で対応しているオブジェクトだと説明していますが、説明だけで「理解できた」とはいきません。そのためまずは確認してみたいと思います。  
...
これで文字列`'ruby'`とシンボル`:ruby`が一対一で対応していることが分かりました。
...
Ruby学習をスタートさせた当時は、あまり疑問に思いもしなかったが改めて「シンボルは文字列の代わりとして使える」という説明を見て次の疑問が浮かびました。  
...
シンボルのメリットでも説明したとおり、速度面や可読性の違いもありますが、それだけではなく文字列とは違いシンボルは必ず同一のオブジェクトになります。


これだけでは分かりづらいと思うので、まずは確認してみましょう。
確認方法は`object_id`メソッドを使います。このメソッドは、そのオブジェクトに対して割り当てられている整数を確認することができます。
